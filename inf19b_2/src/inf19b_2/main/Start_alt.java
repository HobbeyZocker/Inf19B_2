...
//		Paper paper = new Paper("weiß","a4");
//		Paper paper2 = new Paper("Weiß", "A4");
 ...
 * 14) Zu den Aufgaben ist eine entsprechende grafische Benutzeroberfläche zu
 ...
 * Quelldateien (*.java) in einem Zip gepackt über Moodle Es muss eine Klasse
 ...
 * Bewertung fließen funktionale Anforderungen (wie unten beschrieben), aber
 ...
 * Sie haben ein Lager und bekommen Aufträge, Waren einzulagern oder
 * auszulagern. Für erledigte Aufträge bekommen Sie Geld. Ziel des Spiels ist
 * es, so viel wie möglich einzunehmen.
 * 
 * 
 * 
 * Das Lager: Spiel startet, das Lager wird angelegt. Sie besitzen ein
 * Hochregallager mit drei Lagerplätzen Breite und drei Lagerplätzen Höhe. Auf
 * einem Lagerplatz finden drei Paletten hintereinander Platz. Sie haben also
 * insgesamt für 27 Paletten Platz. Ihr Lager ist aktuell leer.
 * 
 * Der Auftrag: Über den Button "Neuer Auftrag" kann ein neuer Auftrag abgerufen
 ...
 * sich immer um eine Einheit. Für die Erzeugung der Aufträge wird eine
 * CSV-Datei bereitgestellt. Wenn das Ende der Datei erreicht ist, kann wieder
 * mit Auftrag 1 begonnen werden. Falls Änderungen / Ergänzungen des CSV Datei
 ...
 * Eigenschaften.
 * 
 * Papier: Eigenschaften: Farbe (Weiß, Grün, Blau), Größe (A3, A4, A5)
 * 
 ...
 * Mittel, Schwer)
 * 
 * Besonderheiten: Schwere Steine sind zu Schwer für das Regal und können nur
 * unten eingelagert werden.
 * 
 * Die Abwicklung: Einlagerungsauftrag: Zu einem Auftrag werden die möglichen
 * Lagerplätze dargestellt, also freie Lagerplätze. Nach Wahl eines Lagerplatzes
 * wird die Palette eingelagert und die Belohnung gutgeschrieben.
 * 
 * Auslagerungsauftrag: Zu einem Auftrag werden die möglichen Lagerplätze
 * dargestellt, also Lagerplätze, an dem genau dieses Produkt eingelagert ist.
 * Nach Wahl eines Lagerplatzes wird die Palette ausgelagert und die Belohnung
 * wird gutgeschrieben
 * 
 * Die Lagerverwaltung: Es ist möglich, dass ein Produkt zwar im Lager vorhanden
 ...
 * die gewünschte Palette ausgewählt. Mögliche Ziel-Lagerplätze werden
 * dargestellt. Nach Wahl eines Ziel-Lagerplatzes wird die Umlagerung
 * durchgeführt. Umlagerungen kosten 100 €. Des weiteren kann ein Produkt auch
 * verschrottet werden, das kostet 500€.
 * 
 * Das Auftragsmanagement: Es können maximal drei Aufträge gleichzeitig
 * bearbeitet werden (weitere Klicks auf "Neuer Auftrag"). So können Sie einen
 * Auftrag auch zunächst einmal zurückstellen. Ein Auftrag kann auch abgelehnt
 * werden, allerdings wird die Belohnung als Vertragsstrafe vom Kontostand
 * abgezogen.
 * 
 * Die Bilanz: Natürlich muss immer der Kontostand sichtbar sein. Zusätzlich
 * gibt es einen Button "Bilanz", der folgende Informationen anzeigt: Umsätze
